...
| Platform patience | After 20 sim-min grace: ≥360 sim-s wait on a connected platform (or ≥900 sim-s if the station has no line) loses; blink in the last 30 s |
...
Bankruptcy uses wall-clock seconds from `Game::Update` (not simulation time scale). Pause freezes the timer. Platform patience uses **simulation time** (scales with game speed) and ignores waits during the first `PlatformPatienceGraceSeconds` (1200 s); wait time for lose/warning is measured from `max(platformArrival, grace end)`. Connected stations use a 360 s limit; stations with no line use 900 s. **Never lose** disables both game-over paths while keeping costs.
